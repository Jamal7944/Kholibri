...
def SelectExo():  # Récupère toutes les données de la table Exercice et les affiche dans le terminal
    ...
    try:  # nécessité de mettre la connexion à la base dans une clause try et de renvoyer l'erreur
        conn = sqlite3.connect(
            'SGBD/BDProjetTut.db')  # la connexion à la base BDProjetTut.db créée un objet de type SQLite connexion
...
def InsertExo(Enonce, Corrige,
              Auteur):  # permet d'ajouter dans la table Exercice des données par le biais de requêtes préparées
    try:
        ...
        data = [Enonce, Corrige, str(time),
                Auteur]  # récupération des données pour les insérer dans l'ordre précis indiqué
...
def InsertShortcuts(command, autor, shortcut):  # meme principe que SelectExo
    try:
        ...
        if error.__class__ == sqlite3.IntegrityError:
            raise RuntimeError("L'auteur et/ou la préférence existe(nt) déjà.")
        else:
            raise RuntimeError("La préférence n'a pas pu être ajoutée.")
...
def giveAuteur():  # meme principe que giveClasse pour les auteurs
    try:
        conn = sqlite3.connect('SGBD/BDProjetTut.db')
        sql = '''Select DISTINCT IdAuteur from AUTEUR  ; '''
        conn.row_factory = lambda cursor, row: row[0]
        cur = conn.cursor()
        res = cur.execute(sql).fetchall()
        cur.close()
...
def ModifyShortcutToCommand(enonce, autor,
                            corrige):  # modifie les enonces et les corriges pour enlever les raccourcis d'un auteur.
    try:
        conn = sqlite3.connect('SGBD/BDProjetTut.db')
        conn.execute("PRAGMA foreign_keys = ON")
...
def ModifyCommandToShortcut(enonce, autor,
                            corrige):  # meme principe que ModifyShortcutToCommand mais dans le sens invers
...
def CreateTableTempLevels():  # meme principe avec la table temporaire CreateTableTempChap
    try:
        conn = sqlite3.connect('SGBD/BDProjetTut.db')
        sql = '''CREATE TABLE IF NOT EXISTS "TempQueryLevels" ("Levels" INTEGER NOT NULL );'''
        cur = conn.cursor()
        cur.execute(sql)
        conn.commit()
...
def InsertAUTEUR(auteur,etablissement):
    try:
        conn = sqlite3.connect('SGBD/BDProjetTut.db')
        conn.execute("PRAGMA foreign_keys = ON")
        sql = ''' Insert into AUTEUR(IdAuteur,Etablissement) VALUES(?,?);'''
        cur = conn.cursor()
        data = [auteur,etablissement]
        cur.execute(sql, data)
        conn.commit()
        cur.close()
        conn.close()
...
def fusion(newchap, classe):
    try:
        ...
        listexo = conn.execute(
            ''' select DISTINCT ExoUtilChap.IdExo from ExoUtilChap where ExoUtilChap.NomChap in (SELECT * FROM TempQueryChap)''').fetchall()
